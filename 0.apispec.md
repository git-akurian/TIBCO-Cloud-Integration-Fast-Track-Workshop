...
*(In TCI, user has the option to directly input schema or to generate schema from sample JSON payloads)*
...
Alternatively, you can skip these configuration steps in building out an API Spec and instead import the completed API spec from a file.

## Importing API Spec Specification
TCI also supports exporting/importing API specifications. If an API has the same name and version that already exists within the team (not just within the group you created), it will fail to import or create. The API Specifications are meant to be team developed and tested. 
